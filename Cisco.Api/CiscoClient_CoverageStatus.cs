...
namespace Cisco.Api
{
	public partial class CiscoClient
	{
		...
		public async Task<CoverageStatus> GetCoverageStatusBySerialNumber(string serialNumber, CancellationToken? cancellationToken = null)
			=> (await GetAsync<CoverageStatusCollection>($"sn2info/v2/coverage/status/serial_numbers/{serialNumber}", cancellationToken)).CoverageStatuses.FirstOrDefault();
		...
		public async Task<CoverageSummary> GetCoverageSummaryBySerialNumber(string serialNumber, CancellationToken? cancellationToken = null)
			=> (await GetAsync<CoverageSummaryCollection>($"sn2info/v2/coverage/summary/serial_numbers/{serialNumber}", cancellationToken)).CoverageSummaries.FirstOrDefault();
		...
		public async Task<List<SerialNumberOrderablePid>> GetOrderableProductIdentifiersBySerialNumber(string serialNumber, CancellationToken? cancellationToken = null)
			=> (await GetAsync<OrderablePidCollection>($"sn2info/v2/identifiers/orderable/serial_numbers/{serialNumber}", cancellationToken)).SerialNumberOrderablePids.FirstOrDefault()?.OrderablePids;
	}
}
